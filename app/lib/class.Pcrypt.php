...
class Pcrypt
{
    ...
    public function __construct($blockmode = MODE_ECB, $cipher = 'BLOWFISH', $key = null)
    {
        ...
        if (!file_exists($_SERVER['DOCUMENT_ROOT']."/" ._APP_LIB. "cipher/".$cipher.".php"))
        {
            $this->error("Unknown Cipher ".$cipher);
        }
        include_once $_SERVER['DOCUMENT_ROOT']."/" ._APP_LIB. "cipher/".$cipher.".php";
 ...
 * 암호화
 * 
 * @param string|array $code 데이타
 * @param string|null $parsePw 암호 키
 * @return string
 */
function SMpcrypt_encode($code='', $parsePw=NULL)
{
	if(!empty($code))
	{
		if( empty($parsePw) ){
			$parsePw = defined('ENCRYPT_SECRET') ? ENCRYPT_SECRET : "Dudtndi@2018_06";
		}
		$code = serialize($code);
		$logg = new Pcrypt(MODE_ECB, "BLOWFISH", $parsePw);
		$code = $logg->encrypt($code);
		//$code = base64_encode($code);
		$code = urlsafe_b64encode($code);
	}
	return $code ;
}
/**
 ...
 * @return array
 */
function SMpcrypt_decode($code='', $parsePw=null){
	...
	{ 
		...
		$de_code = urlsafe_b64decode($code);
		$encted_code = new Pcrypt(MODE_ECB, "BLOWFISH", $parsePw);
		if(!empty($de_code)){
			$code = $encted_code->decrypt($de_code);
			$code = unserialize($code) ;
		}
		//parse_str($code, $send_data) ;
	}
	return (array) $code ;	// 배열값으로 보냄
	//return $code ;	// 배열값으로 보냄
}