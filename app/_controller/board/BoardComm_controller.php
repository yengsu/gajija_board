...
class BoardComm_controller extends BoardCommNest_service
{
    ...
    private function read_attachfile( &$data )
    {
    	...
    	if( !empty($data["attach_files"]) ) {
    		...
    		$attach_downcnt = explode(",", $data["attach_downcnt"]) ;
    	}
        ...
        if( !empty($attach_files) )
        {
            ...
            for($i=0; $i<count($attach_files); $i++)
            {
            	...
            	if($attach_files[$i] && is_file($data["attach_path"].$attach_files[$i])){
            		$size = File::format_bytes( filesize($data["attach_path"].$attach_files[$i]) ) ;
            		array_push($data["attachFiles"], array("exist"=>1, "file"=>$attach_files[$i], "original_file"=>$attach_orig_files[$i], "size" => $size, "down_count" => number_format($attach_downcnt[$i]) ));
            	}else{
     ...
     *
     * @param int $serial (게시판 TB의 P.K)
     * @param string $data 파일별 다운로드수( "20,0,5..." ) - ("0번째 파일 다운로수,  1번째 파일 다운로수, 2번째 파일 다운로수"...)
     * @param int $seq $data의 해당파일 다운로드에 배열 index
     * @param string $group_code 그룹코드 ("board" or "comments"...)
     * @param string $class_code 분류코드 ("qna" or "free" or "tipTech" .. )
     * @param string $kind_name 종류 ( ex: 'abcd.pdf' or 'okbag.xls' ...)
     *
     * @return void
     */
    private function set_fileDownload_count( int $serial, string $data, int $seq, string $group_code, string $class_code="", string $kind_name="" )
    {
    	...
    	if( $res )
    	{
    		...
    		{
	    		$datas = explode(',', $data) ;
	    		(int)$datas[$seq] += 1; // 카운트 증가
	    		$put = implode(",", $datas) ;
    		}else{
    			$put = (int)$data + 1; // 카운트 증가
    		}
         ...
         if(REQUEST_METHOD=="POST")
         {
			...
			if( !empty($res["file"]) )
			{
			    ...
			    $put_data = array_merge($put_data, array(
		    		...
		    		"attach_downcnt" => $res_downs // 초기값: "0,0,0..."
			    ));
			}
...
*/
    ...
    public function download()
    {
        ...
        if( empty($data['serial']) || empty($data['bid'])){
            ...
            $this->set_fileDownload_count(
            		$this->routeResult["code"],  // serial(P.K)
            		$data['attach_downcnt'], //파일별 다운로드 수 "10,5,100,0,1..."
            		(int)$_GET['seq'], // $data['attach_downcnt']의 배열로 가공후의 index
            		"board", // group_code
            		$_REQUEST["bid"], // class_code
            		$data['attachFiles'][(int)$_GET['seq']]['original_file'] // kind_name
				);
