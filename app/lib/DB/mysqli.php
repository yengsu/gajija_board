...
class DB_mysqli extends Exception
{
    ...
    public function rawQuery($query, $bindParams = null)
    {
        ...
        // [exception] update author : yengsu lee
        try {
        	$stmt = $this->_prepareQuery();
        } catch (Exception $e) {
        	$this->_transaction_status_check();
        	foreach($e->getTrace() as $trace){
        		if(preg_match('/_controller.php/', $trace['file'])){
        			$errorMessage = "[line:".
          			$trace['line']."] "
          					.str_replace(".php", "", end(explode(DIRECTORY_SEPARATOR, $trace['file'])))
          					."\n(".explode("query:",$e->getMessage())[0].")";
          					break ;
        		}
        	}
        	if( REQUEST_WITH == 'AJAX' ){
        		//die( header('HTTP/1.0 400 '.rawurlencode($e->getMessage())) );
        		die( header('HTTP/1.0 400 '.rawurlencode($errorMessage)) );
        	}else{
        		//echo '<pre>';print_r($e);
        		//echo $e->getMessage() ;
        		echo $errorMessage ;
        ...
        if( !empty($stmt->error) )
        {
        		$this->_transaction_status_check();
        		//throw new Exception("Unprepared Query Failed, ERRNO: ".$this->mysqli()->errno." (".$this->mysqli()->error.")", $this->mysqli()->errno);
        		throw new BaseException("Query Failed, ERRNO: ".$stmt->errno." (".$stmt->error.")", $stmt->errno);
        }
        //------------------------
        ...
        //추가 : 이영수 ( insert 시 insert_id값 반환)
        if((int)$stmt->insert_id) $insert_id = $stmt->insert_id ;
        
        $res = $this->_dynamicBindResults($stmt);
        $this->reset();
        
        //추가 : 이영수 ( update, delete시 결과 리턴값이 없어서 추가햇음)
        //if(!$res) return ($stmt->affected_rows > 0) ;
        if( (int) $insert_id ) return $insert_id ;
        else if(!$res) return ($this->count > 0) ;
     ...
     */
    protected function _determineType1($item)
    {
        switch (gettype($item)) {
            case 'NULL':
            case 'string':
	...
	protected function _determineType($item)
	{
		if( is_numeric($item) ) {
			...
			else if( is_float($item) ) return 'd';
			//else return 'i';
		}
		else if( is_bool($item) ){ return 'i';
		}else if( is_null($item) || is_string($item) ){ return 's';
		}else if( gettype($item) == 'blob' ){ return 'b';
		}
	}
    /**
    ...
    /**
     * auto_increment 된 int정보를 가져와 +1시킴
     *
     * return : (int) AUTO_INCREMENT 증가값
     *
     */
    public function create_insert_id( $TABLE='', $init=1 ){	// $init = 1000; 최초 record등록 값(P.K)
    	 
    	$sql = "SELECT AUTO_INCREMENT FROM INFORMATION_SCHEMA.TABLES t WHERE t.table_schema = ? AND TABLE_NAME=?";
    	$max = $this->rawQuery( $sql, array($this->db, $TABLE) );
    	$max = array_pop($max);
    
    	/* if($max['AUTO_INCREMENT'] >= $init)
    	 $max['AUTO_INCREMENT'] += 1;
    		else
    			$max['AUTO_INCREMENT'] = $init ; */
    
    		if($max['AUTO_INCREMENT'] >= $init)
    			$max['AUTO_INCREMENT'] = $max['AUTO_INCREMENT'];
    			else
    				$max['AUTO_INCREMENT'] = $init ;
    
    				$sql = "alter table ".$TABLE." auto_increment=".(int)$max['AUTO_INCREMENT'] ;
    				$auto_inc_update = $this->query($sql);
    
    				return $max['AUTO_INCREMENT'];
    }
    /* public function multi_query( $sql ){
     ...
     do {
     if ($result = $this->_mysqli->store_result()) {	//while ($row = $result->fetch_row()) {}
     $result->free();
     }
     if ($this->_mysqli->more_results()) {}//else{ $this->rollback();break ; }
    ...
    public function multiQuery( $sql ) {
    	...
    	do {
    		...
    		if ($result = $this->_mysqli->store_result()) {	/* while ($row = $result->fetch_row()) {echo '<pre>';print_r($row);} */
    		...
    		if( (int)$this->_mysqli->insert_id) $Res[$i] = $this->_mysqli->insert_id ;
    	...
    	// error check
    	if( $err = mysqli_error($this->_mysqli)){
    		
    		$this->rollback() ;
    		
    		if( REQUEST_WITH == 'AJAX' ){
    			die( header('HTTP/1.0 400 '.rawurlencode($err)) );
    		}else{
    			//echo $err;
    ...
    /* public function multiQuery( $sql ) {
    	...
    	do {
    		...
    		$ids[] = $this->_mysqli->insert_id ;
    		if ($this->_mysqli->more_results()) {}else{ 
    			//$this->rollback();
    			break ; }
    	} while ($this->_mysqli->next_result());
