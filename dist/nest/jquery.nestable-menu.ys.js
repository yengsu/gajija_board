...
Menu_handle.prototype = {
	...
	editFrm : function(data, thisObj){
		...
		if(data)
		{
			...
			$("#formEdit").find('input[name="mcode"]').val( data.mcode) ;
			$("#formEdit").find('input[name="mtype"]').val( data.mtype) ;
			$("#formEdit").find('select[name="layout"] option[value="'+data.layout+'"]').prop('selected', 'selected') ;
			$("#formEdit").find('input[name="tpl"]').val( data.tpl) ;
			$("#formEdit").find('input[name="title"]').val( data.title) ;
			$("#formEdit").find('input[name="url"]').val( data.url) ;
			$("#formEdit").find('input[name="url_target"]').val( data.url_target) ;
			
			$("#formEdit").find('input:checkbox[name="used"][value="'+data.used+'"]').prop("checked", true);
			$("#formEdit").find('input:checkbox[name="imp"][value="'+data.imp+'"]').prop("checked", true);
			$("#formEdit").find('input[name="attach_top"]').val( data.attach_top) ;
			$("#formEdit").find('input[name="attach_bottom"]').val( data.attach_bottom) ;
			$("#formEdit").find('textarea[name="attach_top_cont"]').val( data.attach_top_cont) ;
			$("#formEdit").find('textarea[name="attach_bottom_cont"]').val( data.attach_bottom_cont) ;
			
			$("#formEdit").find('select[name="grant_read"] option[value="'+parseInt(data.grant_read)+'"]').prop('selected', 'selected') ;
			$("#formEdit").find('select[name="grant_comments"] option[value="'+parseInt(data.grant_comments)+'"]').prop('selected', 'selected') ;
		}
