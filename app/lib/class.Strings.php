...
class Strings
{
     ...
     * @param array $without 필터링 제외할 변수명들
     * @return void
     *
     * @example $vars = array(
     ...
     */
    public static function set_xss_variable( array $variables = array(), array $without = array() )
    {
        if( empty($variables) )
        {
            foreach($_REQUEST as $name => &$val)
            {
                ...
                {
                    if( empty($val) || ctype_space($val) ) $val = '';
                    $val = trim($val) ;
                    
                    //$val = ( empty($val) || ctype_space($val) ) ? '' : self::set_xss_detect( $val ) ;
                    
                    if(!empty($without))
                    {
                        if( ! array_search($name, $without) ){
                            $val = self::set_xss_detect( $val ) ;
                        }
                    }
                    else{
                        $val = self::set_xss_detect( $val ) ;
            ...
            foreach($variables as $method => $vars)
            {
                ...
                for( $i=0, $l=count($vars); $i < $l; $i++)
                {
                    ...
                    if( $var && is_string($var) )
                    {
                        ...
                        //$_REQUEST[$vars[$i]] = $var = ( empty($var) || ctype_space($var) ) ? '' : trim( self::set_xss_detect($var) );
                        if(!empty($without))
                        {
                            if( ! array_search($vars[$i], $without) ){
                                $var = self::set_xss_detect( $var ) ;
                            }
                        }
                        else{
                            $var = self::set_xss_detect( $var ) ;
     ...
     * @param string $addChars 추가로 섞을 문자들
     * @return string
     */
    public static function shuffle_alphaNum($length=6, string $addChars='')
    {
        $array = array_merge(range(0,9),range('A','Z'),range('a','z'));
        if(!empty($addChars)) $array = array_merge($array, array($addChars)) ;
        shuffle($array);
        //return implode('',array_slice($array,0,$length));
        $res = implode('',array_slice($array,0,$length));
