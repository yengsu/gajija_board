...
class Strings
{
	...
	*/
	...
	/**
	 * 문자열에서 범위영역 안의 문자를 추출
	 *
	 * @param string $Str 문자열
	 * @param string $Sep_front 찾을 영역의 시작위치 문자
	 * @param string $Sep_end 찾을 영역의 마지막위치 문자
	 * @param boolean $indent_last 제일안쪽 영역의 문자를 추출할것인지(중첩블럭인경우 최하위의 블럭을 추출)
	 * @return array array( 0=> ???, 1=>???...... )
	 *
	 * @uses 적용패턴: 중첩블럭인경우 최하위의 블럭을 추출  => [^\[\]]++|\G\[(?<item>(?R)*+)\]
	 * @uses 적용패턴: 중첩블럭 모두 => \[(?<item>(?:[^\[\]]++|\[(?1)\])++)\] <br>
	 * 							문자열 : 이름A{사이즈1,사이즈2{(가격)},사이즈3,사이즈4}_이름B{사이즈x,{사이즈y},사이즈z(가격),사이즈e}
	 * 							결과 : array(item	=>	array(2
	 0	=>	사이즈1,사이즈2{(가격)},사이즈3,사이즈4
	 1	=>	사이즈x,{사이즈y},사이즈z(가격),사이즈e
	 )
	 *
	 * @ignore 실험패턴1: [^\{\}]+|\{(?<item>(?R)*)\} 또는 [^\{\}]+|[{](?<item>(?R)*)[}]
	 *
	 * @example [적용1] Strings::get_findStr_between($Str, '{', '}');<br>
	 [문자열] The {수박} 에서 {정말 {소리} 날까?} ㅎㅎ<br>
	 [결과] array('수박', '정말 {소리} 날까?')
	 *
	 * @example [적용2] Strings::get_findStr_between($Str, '{', '}', true);<br>
	 [문자열] The {수박} 에서 {정말 {소리} 날까?} ㅎㅎ<br>
	 [결과] array('수박', '소리')
	 
	 @link 테스트할수있는곳 https://www.phpliveregex.com/#tab-preg-match-all
	 */
	public static function get_findStr_between($Str, $Sep_front, $Sep_end, $indent_last=false)
	{
		if($indent_last === true) $Regex = "[^". $Sep_front . $Sep_end. "]++|\G\\". $Sep_front ."(?<item>(?R)*+)\\". $Sep_end ;
		else
			//$Regex = "[^". $Sep_front . $Sep_end. "]+|[". $Sep_front ."](?<item>(?R)*)[". $Sep_end ."]" ;
			//$Regex = "[". $Sep_front ."](?<item>(?:[^". $Sep_front . $Sep_end. "]++|[". $Sep_front ."](?1)[". $Sep_end ."])++)[". $Sep_end ."]" ;
			$Regex = "\\". $Sep_front ."(?<item>(?:[^\\". $Sep_front . "\\".$Sep_end. "]++|\\". $Sep_front ."(?1)\\". $Sep_end .")++)\\". $Sep_end ;
			preg_match_all('/'. $Regex .'/', $Str, $Result /* , PREG_OFFSET_CAPTURE *//* , PREG_SET_ORDER */);
			
			$redata = array();
			foreach($Result['item'] as $data){
				if( !empty($data) ) $redata[] = trim($data) ;
			}
			unset($Result) ;
	 ...
	 *
	 * @param string $Str 문자열
	 * @param string $Sep_front 찾을 영역의 시작위치 구분자
	 * @param string $Sep_last 찾을 영역의 마지막위치 구분자
	 * @return array array(0 => array('group'=> ???, 'item'=> ???), 1 => array('group'=> ???, 'item'=> ???)... )
	 *
	 * @access [적용패턴] : (?!\s)(?<group>[^\[\]_,]+)?\[(?<item>(?:[^\[\]]++|\[(?2)\])*)\]
	 *
	 * @ignore 실험패턴0 : 콤마로 구분(,) ===> (?!\s)([^,]+[^,\s]+)
	 * @ignore 실험패턴0 : [값] 005{200원}  [결과] array('number'=>005, 'code'=> 200원)  [패턴] /(?<number>[0-9]+)?\_?[{][^}](?<code>[^\}]*)[^{]*[}]/
	 *
	 * @ignore 실험패턴1 : (?<group>[^,_]+|.)\((?<item>(?:[^()]++|\((?2)\))++)\)
	 * @ignore 실험패턴2 : (?P<group>(?!\s)[^,_].*?)\{(?P<item>.+?)\}
	 * @ignore 실험패턴3:  (?P<group>(?!\s)[^,].*?)(?!\s)([(](?P<item>.+?)[)]|[^,\(\)]+) 또는 (?P<group>(?!\s)[^,].*?)(?!\s)(\((?P<item>.+?)\)|[^,\(\)]+)
	 * @ignore 실험패턴4: ([^{}_]+|<name>+)(?:[^{}]+|\{(?<item>(?R)*)\})
	 *
	 * @example  Strings::get_findStr_2Sep_type1($Str, '(', ')'); <br>
	 * <br>
	 [문자열] 크기1(대규모),크기2(), 크기3(소규모) <br>
	 [문자열] 문자{100},문자{200},문자{300} <br>
	 [문자열] 채소[고구마],야채[상추],과일[사과] <br>
	 [문자열] 이름A{사이즈1,사이즈2{{({가격})}},사이즈3,사이즈4}_이름B{ } <br>
	 [문자열] 이름A[]_이름B[],이름[사이즈z(가격),사이즈e] <br>
	 [문자열] 이름A(aaa1(100),aaa2(200))_이름B(bbb1(100)),이름C(ccc1(100),ccc2(200)),이름C() <br>
	 .....
	 */
	public static function get_findStr_2Sep_type1( $Str, $Sep_front, $Sep_last )
	{
	    $Result = null ;
		...
		"(?<item>".
		      "(?:".
		          "[^". $Sep_front . $Sep_last ."]++|". $Sep_front ."(?2)". $Sep_last .
		      ")".
		"*)".
		$Sep_last ;
		/*
		 // 문자열 끝에 구분자 존재하는지 체크후 구분자 삽입
		 if( phpversion() >= 7.1 ) {
		 $end_char = $Str[-1] ;
		 }
		 // php7.1이하 버전이면
		 else{
		 if( Strings::is_multibyte($Str) ) $end_char = mb_substr($Str, -1) ;
		 else $end_char = substr($Str, -1) ;
		 }
		 if($end_char != $Sep) $Str .= $Separator ; */
		//--------------------------
		...
		foreach($Result as &$datas){
			foreach($datas as $k => &$data){
				if( is_int($k) ) unset($datas[$k]);
				else $data = trim($data) ;
			}
			if( !array_key_exists('item', $datas) ) $datas['item'] = '';
		}
	 ...
	 * @deprecated
	 */
	public static function get_findStr_2Sep_type2()
	{
		//https://stackoverflow.com/questions/16258143/get-all-nested-curly-braces
		//https://stackoverflow.com/questions/15233953/php-split-string-on-comma-but-not-when-between-braces-or-quotes
	}
	/* public static function xss_detect($str)
	{
		$str = strip_tags($str) ; // php, 태그제거
		$str = filter_var($str, FILTER_SANITIZE_STRING); // 문자열에서 태그 / 특수 문자를 제거합니다
		$str = htmlentities($str, ENT_QUOTES, "UTF-8");
	} */
	
	/**
	 * 문자열중의 태그 블럭사이의 내용을 뽑아옴
	 * 
	 * @param string $Str 문자열
	 * @param string $TagName 태그명
	 * @return array|NULL array('attributes'=> 속성, 'item'=> 내용)
	 *
	 * @access [적용패턴] : /(<code[^>]*>)((?:[^\<\>]++|<code[^>]*>(?2)<\/code>)++)(<\/code>)/
	 * 					결과 => array( 1=> "<code....>", 2=> "블럭안의 내용", 3=> "</code>" )
	 *  
	 * @access [적용패턴2] : /<code[^>]*>((?:[^\<\>]++|<code[^>]*>(?1)<\/code>)++)<\/code>/
	 * 					결과 => array(1=> code블럭안의 내용만 추출)
	 * 
	 * @ignore 실험패턴0 : /<code[^>](?P<attributes>.*?)>(?P<item>(?:(?!<\/code(.*?)>)[\s\S])*)<\/code>/si
	 * @ignore 실험패턴1 : <code(?P<attributes>.*?)>(?P<item>(?:(?!</code(.*?)>)[\s\S])*)</code>
	 * @ignore 실험패턴2 : (?P<start>.*?)(<code[^>]*>(?P<item>(?:(?!</code(.*?)>)[\s\S])*)</code>)+(?P<end>.*)
	 * 
	 * @example Strings::get_findStr_tag_between($Str, "code") ;
	 * <br>
	 *     [문자열] <code class="language-css"> ...... </code>
	 */
	public static function get_findStr_tag_between( string $Str, string $TagName)
	{
	    $Result = null ;
	    //$Regex = "/<" . $TagName."[^>](?P<attributes>.*)>(?P<item>(?:(?!<\/". $TagName ."(.*?)>)[\s\S])*)<\/". $TagName .">/si" ;
	    $Regex = "/(<" . $TagName. "[^>]*>)((?:[^\<\>]++|<" . $TagName. "[^>]*>(?2)<\/" . $TagName. ">)++)(<\/" . $TagName. ">)/" ;
	 ...
	 * 
	 * @param string $str
	 * @param string $attr_name 찾을 속성명 (n개 일수도 있음)
	 * @return array
	 */
	public static function get_attributes( string $str, $attr_name='' )
	{
		$Regex = '';
		if(!empty($attr_name)) $Regex = '/('.$attr_name.')(?:\s?)+=(?:\s?)+([^"\']+|([\'"]?)(?:[^\3]|\3+)+?\3)/si' ;
		else $Regex = '/([\w\-]+)(?:\s?)+=(?:\s?)+([^"\']+|([\'"]?)(?:[^\3]|\3+)+?\3)/si' ;
