...
class GDimage {
    ...
    //function GDimage($file, $lang = 'en_GB') {
        ...
        /* $ translation  =  array ();
        $ translation [ ' file_error ' ]                   =  ' 파일 오류 : 다시 실행하십시오. ' ;
        $ translation [ ' local_file_missing ' ]           =  ' 로컬 파일이 존재하지 않습니다. ' ;
        $ translation [ ' local_file_not_readable ' ]      =  ' 로컬 파일의 읽기에 실패했습니다. ' ;
        $ translation [ ' uploaded_too_big_ini ' ]         =  ' 파일 업로드에 실패했습니다. (the uploaded file exceeds the upload_max_filesize directive in php.ini). ' ;
        $ translation [ ' uploaded_too_big_html ' ]        =  ' 파일 업로드에 실패했습니다. (the uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the html form) ' ;
        $ translation [ ' uploaded_partial ' ]             =  ' 파일 업로드에 실패했습니다. (the uploaded file was only partially uploaded). ' ;
        $ translation [ ' uploaded_missing ' ]             =  ' 파일 업로드에 실패했습니다. (no file was uploaded). ' ;
        $ translation [ ' uploaded_no_tmp_dir ' ]          =  ' 파일 업로드에 실패했습니다. (missing a temporary folder) ' ;
        $ translation [ ' uploaded_cant_write ' ]          =  ' 파일 업로드에 실패했습니다. (failed to write file to disk) ' ;
        $ translation [ ' uploaded_err_extension ' ]       =  ' 파일 업로드에 실패했습니다. (file upload stopped by extension) ' ;
        $ translation [ ' uploaded_unknown ' ]             =  ' 파일 업로드에 실패했습니다. (unknown error code) ' ;
        $ translation [ ' try_again ' ]                    =  ' 파일 업로드에 실패했습니다. 다시 실행하십시오. ' ;
        $ translation [ ' file_too_big ' ]                 =  ' 파일이 너무 큽니다. ' ;
        $ translation [ ' no_mime ' ]                      =  ' MIME 유형을 찾을 수 없습니다. ' ;
        $ translation [ ' incorrect_file ' ]               =  ' 파일 형식이 다릅니다. ' ;
        $ translation [ ' image_too_wide ' ]               =  ' 이미지 넓은습니다. ' ;
        $ translation [ ' image_too_narrow ' ]             =  ' 이미지가 너무 좁게합니다. ' ;
        $ translation [ ' image_too_high ' ]               =  ' 이미지 폭이 너무 높습니다. ' ;
        $ translation [ ' image_too_short ' ]              =  ' 이미지 폭이 너무 낮습니다. ' ;
        $ translation [ ' ratio_too_high ' ]               =  ' 이미지 비율이 너무 높습니다. (이미지 폭이 너무 넓합니다) ' ;
        $ translation [ ' ratio_too_low ' ]                =  ' 이미지 비율이 너무 낮습니다 (이미지 폭이 너무 높습니다) ' ;
        $ translation [ ' too_many_pixels ' ]              =  ' 이미지 픽셀 수가 너무 많습니다. ' ;
        $ translation [ ' not_enough_pixels ' ]            =  ' 이미지 픽셀 수가 너무 적습니다. ' ;
        $ translation [ ' file_not_uploaded ' ]            =  ' 파일을 업로드 할 수 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' already_exists ' ]               =  ' % s는 이미 존재합니다. 파일 이름을 변경하십시오. ' ;
        $ translation [ ' temp_file_missing ' ]            =  ' 임시 파일이 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' source_missing ' ]               =  ' 업로드 파일이 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' destination_dir ' ]              =  ' 대상 디렉터리를 만들 수 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' destination_dir_missing ' ]      =  ' 대상 디렉토리가 존재하지 않습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' destination_path_not_dir ' ]     =  ' 이동 경로는 디렉토리가 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' destination_dir_write ' ]        =  ' 대상 디렉토리를 쓸 수 수 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' destination_path_write ' ]       =  ' 이동 경로는 쓸 수는 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' temp_file ' ]                    =  ' 임시 파일을 만들 수 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' source_not_readable ' ]          =  ' 원본 파일을로드 할 수 없습니다. 프로세스를 실행 할 수 없습니다. ' ;
        $ translation [ ' no_create_support ' ]            =  ' % s 이미지를 만들 수 없습니다. ' ;
        $ translation [ ' create_error ' ]                 =  ' 이미지 소스에서 % s 이미지를 만들 수 없습니다. ' ;
        $ translation [ ' source_invalid ' ]               =  ' 이미지 소스를 읽을 수 없습니다. ' ;
        $ translation [ ' gd_missing ' ]                   =  ' GD가 존재하지 않습니다. ' ;
        $ translation [ ' watermark_no_create_support ' ] =  ' 워터 마크를 읽을 수 없기 때문에 % s 이미지를 만들 수 없습니다. ' ;
        $ translation [ ' watermark_create_error ' ]       =  ' % s의 읽기는 지원되지 않습니다. 워터 마크를 만들 수 없습니다. ' ;
        $ translation [ ' watermark_invalid ' ]            =  ' 이미지 형식을 알 수 없습니다. 워터 마크를 읽을 수 없습니다. ' ;
        $ translation [ ' file_create ' ]                  =  ' % s 이미지를 만들 수 없습니다. ' ;
        $ translation [ ' no_conversion_type ' ]           =  ' 변환 형식이 정의되어 있지 않습니다. ' ;
        $ translation [ ' copy_failed ' ]                  =  ' 파일 복사 오류. copy ()에 실패했습니다. ' ;
        $ translation [ ' reading_failed ' ]               =  ' 파일 읽기 오류. ' ; */
