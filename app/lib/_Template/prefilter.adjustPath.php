...
function adjustPath($source, $tpl, $indicator='', $type='absolute')
{
	...
	$path_filter = array();
	$path_filter = array(
			//'/html/_layout/(.*'=>'http://'.$_SERVER['HTTP_HOST'].'/',
			//'@^/html/_layout/img/(.*)\.(gif|jpg)$@' => '/imgA/$1.$2' 
			//'@^/html/_layout/(.*/)([^/]+)/?\.(gif|jpg)$@' => '/$1/$2.$3'
			//'@^(.*/)([^/]+)/?\.(gif|jpg)$@' => '/sss/$1/$2.$3'
			
			//'@^(/html/_layout(.*))(images|css)/(.*/)([^/]+)/?\.(gif|jpg|png)$@' => '/$2/$3/$4.$5'
			//'@/html/_layout/adm/(.*)\.(gif|jpg|png)$@' => '/$1.$2'
	); 
