...
CREATE TABLE `board` (
  ...
  `attach_orig_files` varchar(255) CHARACTER SET utf8 DEFAULT '' COMMENT '첨부 원본파일명(파일,파일,파일,파일.....)',
  `attach_downcnt` varchar(150) COLLATE utf8_unicode_ci DEFAULT '' COMMENT '각 첨부파일 다운로드 횟수(15,0,0,35...)',
...
CREATE TABLE `board_info` (
  ...
  `oid` int(11) DEFAULT '0' COMMENT '업체코드',
  `bid` varchar(35) CHARACTER SET utf8 NOT NULL DEFAULT '' COMMENT '게시판 ID (uniq)',
  `mcode` int(11) DEFAULT '0' COMMENT '메뉴(분류)코드',
...
CREATE TABLE `comments_info` (
  `serial` int(11) unsigned NOT NULL AUTO_INCREMENT,
  `oid` int(11) DEFAULT '0' COMMENT '업체코드',
  `bid` varchar(35) CHARACTER SET utf8 DEFAULT '' COMMENT '게시판 아이디',
...
CREATE TABLE `viewcnt` (
  ...
  `kind_name` varchar(150) DEFAULT '' COMMENT '종류',
  `serial_code` int(11) unsigned DEFAULT '0' COMMENT '테이블의 serial코드',
  `ip` varchar(20) NOT NULL DEFAULT '',
  `regdate` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`serial`),
  UNIQUE KEY `unique_col` (`oid`,`group_code`,`class_code`,`kind_name`,`serial_code`,`ip`),
  KEY `idx_group` (`oid`,`group_code`),
  KEY `idx_class` (`oid`,`group_code`,`class_code`),
  KEY `idx_oid` (`oid`),
  KEY `idx_regdate` (`regdate`),
  KEY `idx_ip` (`ip`),
  KEY `idx_serial` (`oid`,`group_code`,`class_code`,`kind_name`,`serial_code`),
  KEY `idx_serial2` (`oid`,`group_code`,`class_code`,`serial_code`),
  KEY `idx_kind_name` (`kind_name`)
