...
class BikeRouter
{
    ...
    /* use Singleton	// trait 사용
    {
    	getInstance as singleton ;	// 이름 재정의
    } */
    /**
     * Constructor
     *
     * @param array $routes - predefined routes array
     */
    public function __construct( $routes = 'default' )
    {
    	//$argv = func_get_args();
    	...
    	$this->routes = self::dsn($routes) ;
     	...
     	return $this->match( 'GET, POST', REQUEST_URI );
    }
    
    public static function dsn($conf) {
    	//echo "router.${conf}";
    	//$info = core\WebApp::getConf("router.${conf}");
    	$info = \WebApp::getConf("router.${conf}");
